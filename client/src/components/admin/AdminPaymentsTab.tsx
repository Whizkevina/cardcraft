...
function VerifyButton({ payment }: { payment: AdminPayment }) {
  const { toast } = useToast();
  const qc = useQueryClient();

  const verify = useMutation({
    mutationFn: async () => {
      const r = await apiRequest("POST", `/api/admin/payments/${payment.id}/verify`);
      const body = await r.json();
      if (!r.ok) throw new Error(body?.error || "Verification failed");
      return body as { reconciled: boolean; paystackStatus: string | null };
    },
    onSuccess: (result) => {
      qc.invalidateQueries({ queryKey: ["/api/admin/payments"] });
      qc.invalidateQueries({ queryKey: ["/api/admin/audit-log"] });
      toast({
        title: result.reconciled ? "Payment reconciled" : "No change needed",
        description: result.paystackStatus ? `Paystack reports: ${result.paystackStatus}` : undefined,
      });
    },
    onError: (err: Error) => toast({ title: "Could not verify", description: err.message, variant: "destructive" }),
  });
  ...
  return (
    ...
    >
      <RefreshCw size={11} className={verify.isPending ? "animate-spin" : ""} />
      Verify
    </Button>
  );
}
